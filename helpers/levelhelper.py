...
class LevelHelper:
    ...
    async def _create_level_obj(self, level_id: int) -> Level:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "SELECT gameVersion,binaryVersion,userName,levelID,levelName,levelDesc,levelVersion,levelLength,audioTrack,password,original,twoPlayer,songID,objects,coins,requestedStars,levelInfo,extraString,starStars,uploadDate,updateDate,starCoins,starFeatured,starEpic,starDemonDiff,userID,extID,isLDM,downloads,likes FROM levels WHERE levelID = %s LIMIT 1",
                (level_id,),
            )
    ...
    def star_to_difficulty(self, star_count: int) -> int:
        ...
        return {1: 0, 2: 10, 3: 20, 4: 30, 5: 30, 6: 40, 7: 40, 8: 50, 9: 50}.get(
            star_count, 0
        )
    ...
    async def multi_gen(self, levels: list) -> str:
        ...
        for level in levels:
            ...
            level = await self.get_level_obj(
                level
            )  # Use this as we are expecting them to be already cached.
    ...
    async def bump_download(self, level_id: int):
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "UPDATE levels SET downloads = downloads + 1 WHERE levelID = %s",
                (level_id,),
            )
    ...
    async def upload_level(self, level: Level) -> int:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "SELECT levelID FROM levels WHERE extID = %s AND levelName = %s",
                (level.account_id, level.name),
            )
            ...
            if level_count is not None:
                ...
                await myconn.conn.commit()
                os.remove(
                    user_config["level_path"] + str(level_id)
                )  # Removing previous level version
            ...
            async with AIOFile(
                user_config["level_path"] + str(level_id), "w+"
            ) as file:  # tbh this should be a function
    ...
    async def rate_level(self, rating: Rating) -> None:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "UPDATE levels SET starStars = %s, starFeatured=%s,starEpic = %s, starCoins=%s,starDemonDiff=%s WHERE levelID = %s LIMIT 1",
                (
                    rating.stars,
                    int(rating.featured),
                    int(rating.epic),
                    int(rating.verified_coins),
                    rating.demon_diff,
                    rating.level_id,
                ),
            )
    ...
    async def _daily_level_from_db(self) -> DailyLevel:
        ...
        return DailyLevel(daily[0], daily[1], int(daily[2]), bool(daily[3]))
...
level_helper = (
    LevelHelper()
)  # Shared object between all imports for caching to work correctly etc.
