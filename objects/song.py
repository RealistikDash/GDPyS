...
from utils.gdform import parse_to_dict
from web.client import post_request
from web.client import simple_get


class Song:
    """The object representation of the GDPyS and Geometry Dash songs."""

    __slots__ = ("id", "title", "author_id", "author_name", "size", "author_yt", "url")
    ...
    async def from_boomlings(cls, song_id: int):
        ...
        boomlings_data = await post_request(
            ...
            {"secret": "Wmfd2893gb7", "songID": song_id},
        )
        ...
        if is_numeric(boomlings_data) or not boomlings_data:
            error(
                f"Boomlings Private Song API sent an invalid response for the song ID of {song_id}",
            )
            return

        # New boomlings firewall
        if (
            "Cloudflare to restrict access" in boomlings_data
            or "error code" in boomlings_data
        ):
            return error(
                f"Boomlings Private Song API request has been blocked by the "
                "Cloudflare firewall.",
            )
    ...
    async def from_newgrounds(cls, song_id: int):
        ...
        if "No Audio Project exists with ID" in newgrounds_resp:  # XXX: Comments.
            error(
                f"Newgrounds Request for song with ID {song_id} failed! "
                "Song not Found.",
            )
            return
        ...
        self.url = (
            Regexes.NG_SONG_LINK.search(newgrounds_resp).group(1).replace("\\/", "/")
        )
    ...
    async def from_id(cls, song_id: int):
        ...
        if not song_id:
            return Song()  # Empty because else causes errors lol. # XXX: what was I on?
