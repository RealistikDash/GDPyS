...
def bcrypt_hash(passw: str, difficulty: int = 10) -> str:
    ...
    return bcrypt.hashpw(passw.encode(), bcrypt.gensalt(difficulty)).decode()
...
def bcrypt_check(plain_pass: str, bcrypt_pass: str) -> bool:
    ...
    try:
        return bcrypt.checkpw(plain_pass.encode(), bcrypt_pass.encode())
...
def gjp_decode(gjp: str) -> str:
    ...
    return xor_cipher(base64_decode(gjp), XorKeys.GJP)
