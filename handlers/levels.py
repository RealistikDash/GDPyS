...
from helpers.crypt import sha1_hash
...
from utils.security import remove_col_tags
from utils.security import verify_comment
...
async def upload_level(req: Request, user: User) -> str:
    ...
    if (
        (l_id := int(req.post["levelID"]))
        and (level := await Level.from_id(l_id))
        and level.creator.id == user.id
    ):
...
async def level_search(req: Request) -> str:
    ...
    #### IGNORE FOR TESTING.
    levels_db = await glob.sql.fetchall(
        BASE_QUERY + f"ORDER BY {order} LIMIT {PAGE_SIZE} OFFSET {offset}",
    )
    ...
    for level in levels_db:
        ...
        # Grab song
        if level[15]:
            song_strs.append((await Song.from_id(level[15])).resp())
        ...
        security_strs.append(f"{level_id[0]}{level_id[-1]}{level[11]}{level[17]}")
...
async def download_level(req: Request) -> str:
    ...
    security_str2 = ",".join(
        (
            str(level.creator.id),
            str(level.stars),
            "1" if level.demon else "0",
            str(level.id),
            "1" if level.coins_verified else "0",
            str(level.feature_id),
            level.password,
            "0",
        ),
    )
    ...
    return "#".join((main_resp, security_str, security_str2_h, security_str2))
...
async def upload_comment(req: Request, u: User):
    ...
    if not verify_comment(content):
        debug(
            f"Comment upload by {u} failed: Comment content check fail (possible bot?).",
        )
        raise GDPySHandlerException("-1")
