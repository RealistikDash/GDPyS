...
async def level_search_modular_hanlder(
    request: aiohttp.web.Request,
) -> aiohttp.web.Response:  # Kinda stole the name for the function from osu lol
    ...
    if not gauntlet_req:
        ...
        gauntlet: Gauntlet = select_obj_id(
            gauntlets, gauntlet_req
    ...
    for level in levels.results if not gauntlet_req else levels:
        ...
        if level.song_id:
            song_str += (
                songs.song_string(await songs.get_song_obj(level.song_id)) + "~:~"
            )
    ...
    response = (
        ...
        + "#"
        + song_str[:-3]
        + f"#{levels.total_results if not gauntlet_req else len(gauntlets)}:{offset}:10#"
        + await level_helper.multi_gen(lvls_list)
    )
...
async def download_level(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    yo_idk = list_comma_string(
        [
            level.user_id,
            level.stars,
            1 if level.stars == 10 else 0,
            level.ID,
            int(level.verified_coins),
            int(level.featured),
            level.password,
            fea_id,
        ]
...
async def get_gauntlets_handler(request: aiohttp.web.Request):
    ...
    for gauntlet in gauntlets:
        gautnlet_resp += (
            joint_string({1: gauntlet.ID, 3: list_comma_string(gauntlet.level_list())})
            + "|"
        )
