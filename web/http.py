...
class Request:
    ...
    def __init__(self, client: socket.socket, loop: asyncio.AbstractEventLoop) -> None:
    ...
    def _parse_multipart(self) -> None:
        ...
        for part in parts[:-1]:
            ...
            temp_headers = {}
            for key, val in [
                p.split(":", 1) for p in headers.decode().split("\r\n")[1:]
            ]:
            ...
            if "filename" in temp_args:
                self.files[temp_args["filename"]] = body[:-2]  # It is a file.
            else:
                self.post[temp_args["name"]] = body[:-2].decode()  # It's a post arg.
    ...
    async def perform_parse(self) -> None:
        ...
        try:
            ...
            return  # Get args request only.

        if to_read := ((offset + 4) + content_len) - len(
            buffer,
        ...
        if self.type == "POST":
            if ctx_type := self.headers.get("Content-Type"):
                if (
                    ctx_type.startswith("multipart/form-data")
                    or "form-data" in ctx_type
                ):
...
class Handler:
    ...
    def verify_postargs(self, args: Union[list, tuple]) -> bool:
        ...
        if not DEBUG:
            return all(arg in args for arg in self.req_postargs)
        else:
            ...
            if missing_args:
                debug(
                    f"Request is missing the post arguments {', '.join(missing_args)}",
                )
                return False
            return True
...
class GDPySWeb:
    ...
    async def config_sql(
        ...
        self.pool = await MySQLPool.connect(host, user, password, database, port)
    ...
    async def _gd_auth(self, req: Request) -> bool:
        ...
        if int(req.post["gameVersion"]) < 21 or int(req.post["binaryVersion"]) < 34:
        ...
        return await self.auth.gjp_check(int(req.post["accountID"]), req.post["gjp"])
    ...
    async def _handle_conn(self, request: Request) -> None:
        ...
        try:
            ...
            if handler.has_status(HandlerTypes.RATE_LIMITED) and not self._rate_limit(
                request,
            ):
                # Idk what to do here....
                pass
            ...
            if isinstance(resp_str, tuple):
                request.resp_code, resp_str = resp_str

        except GDPySHandlerException as e:
            resp_str = str(e).encode()
            debug(
                f"Handler triggered error code {resp_str}",
            )  # Temp debug as else it will be triggered a lot.
        ...
        # There has been an actual exception within the handler.
            ...
            error(
                f"There was an exception when handling path {request.path} using {handler.handler.__name__}\n{tb}",
            )
