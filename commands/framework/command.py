...
class Command:
    def __init__(
        ...
        name: str,
        desc: str,
        priv_req: Optional[Privileges] = None,
    ) -> None:
    ...
    def syntax_advice(self) -> str:
        ...
        return " ".join([f"<{k} : {v.__name__}>" for k, v in self._args_actual()])
    ...
    def _args_actual(self) -> dict:
        ...
        return {
            k: v
            for k, v in self.execute.__annotations__.items()
    ...
    def _adapt_args(self, ctx: CommandContext) -> Optional[list]:
        ...
        resp_args = []
        default_args = (
            list(self.execute.__defaults__) if self.execute.__defaults__ else None
        )

        untouched_args = ctx._processing_content.split(" ")
        for idx, (_, v) in enumerate(self._args_actual().items()):
            try:
                word = v(untouched_args[idx])
            except IndexError:
                # Check if we have a default, else error.
                try:
                    word = default_args.pop(0)  # Wished this was a dict.
    ...
    async def handle(self, ctx: CommandContext) -> Optional[str]:
        ...
        if not ctx._processing_content:
            ctx._processing_content = ctx.content
        ctx._processing_content = ctx._processing_content.removeprefix(self.name)

        # Run checks first.
        if not self._priv_check(ctx):
            return self.on_priv_check_fail()
        if not await self.check_prior(ctx):
            return self.on_custom_check_fail()

        adapted_args = self._adapt_args(ctx)
        if adapted_args is None:
            return self.on_arg_mismatch(ctx)
