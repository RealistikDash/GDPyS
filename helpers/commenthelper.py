...
class CommentHelper:
    ...
    async def get_comments(
        self, level_id: int, page: int = 0, amount: int = 10, order: str = "likes"
    ) -> QueryResponse:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                f"SELECT userID, userName, comment, timestamp, likes, percent, isSpam, levelID, commentID FROM comments WHERE levelID = %s ORDER BY {order} DESC LIMIT %s OFFSET %s",
                (level_id, amount, offset),
            )
            comments_db = await mycursor.fetchall()
            # Count query
            await mycursor.execute(
                "SELECT COUNT(*) FROM comments WHERE levelID = %s", (level_id,)
            )
        ...
        return QueryResponse(count, comment_list)

    async def insert_comment(self, comment: Comment) -> None:
        """Adds comment to database from object."""
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "INSERT INTO comments (userID, userName, comment, levelID, timestamp, likes, percent, isSpam) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                (
                    comment.user_id,
                    comment.username,
                    comment.comment_base64,
                    comment.level_id,
                    comment.timestamp,
                    comment.likes,
                    comment.percent,
