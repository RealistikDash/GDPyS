...
async def cron_calc_cp(conn):
    ...
    async with conn.cursor() as mycursor:
        ...
        await mycursor.execute(
            "SELECT COUNT(*) FROM levels WHERE starStars > 0 OR starFeatured > 0 OR starEpic > 0 OR awarded > 0 OR magic>0"
        )
        ...
        for i in range(pages):
            ...
            await mycursor.execute(
                "SELECT extID, starStars,starFeatured,starEpic,awarded,magic FROM levels WHERE starStars > 0 OR starFeatured > 0 OR starEpic > 0 OR awarded > 0 OR magic>0 LIMIT %s OFFSET %s",
                (PAGE_SIZE, offset),
            )
