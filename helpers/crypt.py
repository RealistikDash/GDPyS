...
def base64_encode(text: str) -> str:
    """Encodes the param `text` using url safe
    Base64.
    
    Args:
        text (str): The text that is to be encoded
            using Base64.
...
def base64_decode(text: str) -> str:
    """Decodes param `text` from Base64 to
    regular string.
    ...
    Return:
        Decoded string of the contents of the
            base64 encoded string.
    """
...
def bcrypt_hash(passw: str, difficulty: int = 10) -> str:
    """Hashes the pasword `passw` using the
    BCrypt hashing algorhythm.

    Args:
        passw (str): The password that is
            supposed to be hashed.
        difficulty (int): The amount of
            times the password should be
            salted (higher is more secure
            but slower).
...
def xor_cipher(text: str, key: int) -> str:
    """Encrypts the string `text` using the XOR 
    cipher with the key `key`.
    
    Args:
        text (str): The text that is to be encrypted
            using the XOR cipher.
        key (int): The key to be used within the
            encryption process.
    
    Returns:
        An XOR encoded string of param `text` using
            the key `key`.
    """
...
def gjp_decode(gjp: str) -> str:
    ...
    Args:
        gjp (str): The Geometry Jump Password
...
def string_random(length: int = 8) -> str:
    ...
    Note:
        The string generated will only contain
            ASCII upper case and lower case
            characters.
