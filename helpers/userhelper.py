...
class UserHelper():
    ...
    async def _create_user_object(self, account_id: int) -> Account:
        ...
        for comment in comments:
            acc_comments.append(AccountComment(comment[0], comment[2], decode_base64(
                comment[2]), comment[3], comment[4], bool(comment[5]), comment[1], comment[6]))
    ...
    # May add some pagination system directly to the sql for speed ig.
