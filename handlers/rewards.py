...
async def quests_handler(request: aiohttp.web.Request):
    ...
    user = await user_helper.get_object(
    ...
    quests = (
        await rewards_helper.get_quests()
    )  # This is so we can pop quests we already used without messing up the global quests.
    quest_str = []

    for i in range(
        3
    ):  # Get  3 raandom quests. This is a basic form of quest differentiation and is prob due for a redo
