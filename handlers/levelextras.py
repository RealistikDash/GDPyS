...
async def level_comments_handler(request: aiohttp.web.Request):
    ...
    comments = await comment_helper.get_comments(
        level_id, page, amount, "likes" if mode else "timestamp"
    )
    ...
    for comment in comments.results:
        comment: Comment
        try:
            comment_user = await user_helper.get_object(
                await user_helper.userid_accid(comment.user_id)
            )
        except AssertionError:  # The user does not exist
            logging.debug(lang.debug("comment_user_search_fail", comment.user_id))
        else:
            privilege = await priv_helper.get_privilege_from_privs(
                comment_user.privileges
            )
            response += (
                wave_string(
                    {
...
async def post_comment_handler(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    # TODO : Command stuff
    if comment_obj.comment.startswith(
        user_config["command_prefix"]
    ) and client._command_exists(comment_obj.comment):
...
async def level_scores_handler(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    if await score_helper.overwrite_score(score=score) and score.percentage > 0:
        ...
        old_score = await score_helper.get_score_for_user(
            score.account_id, score.level_id
        )
    ...
    lbs_get = {1: score_helper.get_from_db}.get(  # Budget switch statement.
        lb_type, score_helper.get_from_db
    )
