...
try:
    ...
    logger.error(
        f"{Fore.RED}[GDPyS] Failed connecting to MySQL! Aborting!\nError: {e}{Fore.RESET}"
    )
    exit()
...
LevelDLCache = (
    {}
)  # the level data will be stored here so it doesnt have to be redownloaded
...
def get_bcrypt_password(AccountID):
    ...
    if AccountID not in list(BrcyptCache.keys()):
        mycursor.execute(
            "SELECT password FROM accounts WHERE accountID = %s LIMIT 1", (AccountID,)
        )
...
def login_check(Udid, Username, Password, request):
    ...
    mycursor.execute(
        "SELECT accountID FROM accounts WHERE userName LIKE %s", (Username,)
    )
    ...
    # User ID
    mycursor.execute(
        "SELECT userID, isBanned FROM users WHERE extID = %s", (AccountID,)
    )
...
def register_function(request):
    ...
    mycursor.execute(
        "SELECT userName FROM accounts WHERE userName LIKE %s LIMIT 1",
    ...
    # Query Time
    mycursor.execute(
        "INSERT INTO accounts (userName, password, email, secret, saveData, registerDate, saveKey) VALUES (%s, %s, %s, '', '', %s, '')",
...
def get_user_data_fsunction(request):
    ...
    # checking for blocks
    mycursor.execute(
        "SELECT id FROM blocks WHERE (person1 = %s AND person2 = %s) OR (person1 = %s AND person2 = %s)",
        (TargetAccid, FromAccid, FromAccid, TargetAccid),
    )
    ...
    if TargetAccid == FromAccid:  # its the user checking themselves out
        ...
        mycursor.execute(
            "SELECT count(*) FROM friendships WHERE (person1 = %s AND person2 = %s) OR (person2 = %s AND person1 = %s)",
            (TargetAccid, FromAccid, FromAccid, TargetAccid),
        )
        if mycursor.fetchone()[0] > 0:
            ...
            mycursor.execute(
                "SELECT ID, comment, uploadDate FROM friendreqs WHERE accountID = %s AND toAccountID = %s",
                (TargetAccid, FromAccid),
            )
    ...
    mycursor.execute(
        "SELECT youtubeurl, twitter, twitch, frS, mS, cS FROM accounts WHERE accountID = %s LIMIT 1",
        (TargetAccid,),
    )
...
def get_acc_comments(request):
    ...
    mycursor.execute(
        "SELECT comment, userID, likes, isSpam, commentID, timestamp FROM acccomments WHERE userID = %s ORDER BY timeStamp DESC LIMIT 10 OFFSET %s",
        (UserID, Offset),
    )
...
def insert_acc_comment(request):
    ...
    mycursor.execute(
        "INSERT INTO acccomments (comment, userID, timeStamp, userName) VALUES (%s, %s, %s, %s)",
        (CommentContent, USerID, Timestamp, Username),
    )
    mydb.commit()
    return "1"
...
def update_acc_Settings(request):
    ...
    # setting the things
    mycursor.execute(
        "UPDATE accounts SET mS = %s, cS = %s, frS = %s, youtubeurl = %s, twitter = %s, twitch = %s WHERE accountID = %s",
        (Ms, Cs, Frs, YouTube, Twitter, Twitch, AccountID),
    )
    mydb.commit()
...
def update_user_score(request):
    ...
    mycursor.execute(
        "SELECT accountID FROM accounts WHERE userName LIKE %s",
        (request.form["userName"],),
    )
    ...
    mycursor.execute(
        ...
        (
            ...
            DataDict["accGlow"],
            DataDict["accSpider"],
            DataDict["accExplosion"],
            DataDict["gameVersion"],
            DataDict["secret"],
            DataDict["coins"],
            DataDict["userCoins"],
            DataDict["icon"],
...
def get_leaderboards(request):
    ...
    LeaderboardType = request.form[
        "type"
    ]  # 4 leaderboard types, realitive, creator, friends and top
    ...
    if LeaderboardType == "top":
        ...
        mycursor.execute(
            "SELECT * FROM users WHERE isBanned = '0' AND stars > 0 ORDER BY stars DESC LIMIT 100"
        )

    elif LeaderboardType == "creators":
        mycursor.execute(
            "SELECT * FROM users WHERE isCreatorBanned = '0' AND isBanned = '0' ORDER BY creatorPoints DESC LIMIT 100"
        )
...
def rewards(request):
    ...
    Timestamp = round(time.time())
    RewardType = request.form[
        "rewardType"
    ]  # 0 = get time, 1= small chest, 2 = big chest
    ...
    mycursor.execute(
        "SELECT chest1count, chest1time, chest2count, chest2time FROM users WHERE userID = %s LIMIT 1",
        (UserID,),
    )  # get chest 1 data
    ...
    if RewardType == 1:
        ...
        mycursor.execute(
            "UPDATE users SET chest1count = %s, chest1time = %s WHERE userID = %s LIMIT 1",
            (NewChestCount, NewChestTime, UserID),
        )
    ...
    if RewardType == 2:
        ...
        mycursor.execute(
            "UPDATE users SET chest2count = %s, chest2time = %s WHERE userID = %s LIMIT 1",
            (NewChestCount, NewChestTime, UserID),
        )
...
def SaveUserData(request):
    ...
    mycursor.execute(
        "UPDATE users SET orbs = %s, completedLvls = %s WHERE extID = %s",
        (OrbCount, LevelCount, AccountID),
    )
    mydb.commit()

    # replacing  the user's password
    SaveData = SaveData.replace(
        f"<k>GJA_002</k><s>{Password}</s>", "<k>GJA_002</k><s>password go bye bye</s>"
    )
...
def like_function(request):
    ...
    mycursor.execute(
        f"SELECT likes FROM {Table} WHERE {Column} = %s LIMIT 1", (ItemID,)
    )
    ...
    mycursor.execute(
        f"UPDATE {Table} SET likes = %s WHERE {Column} = %s", (Likes, ItemID)
    )
    mydb.commit()
    return "1"
...
def gen_multi(LevelMultiString):
    ...
    for Level in Levels:
        mycursor.execute(
            "SELECT levelID, starStars, starCoins FROM levels WHERE levelID = %s",
            (Level,),
        )
        Data = mycursor.fetchall()
        if len(Data) > 0:
...
def gen_song_string(SongID: int):
    ...
    mycursor.execute(
        "SELECT ID, name, authorID, authorName, size, isDisabled, download FROM songs WHERE ID = %s",
        (SongID,),
    )
...
def get_levels(request):
    ...
    if Gauntlet != 0:
        mycursor.execute(
            "SELECT level1, level2, level3, level4, level5 FROM gauntlets WHERE ID = %s LIMIT 1",
            (Gauntlet,),
        )
        ...
        if GauntletLevels == None:
            ...
            SQLFormats.append(
                f"{GauntletLevels[0]},{GauntletLevels[1]},{GauntletLevels[2]},{GauntletLevels[3]},{GauntletLevels[4]}"
            )
...
def get_song(request):
    ...
    mycursor.execute(
        "SELECT ID, name, authorID, authorName, size, isDisabled, download FROM songs WHERE ID = %s LIMIT 1",
        (SongID,),
    )
    SongData = mycursor.fetchall()
    if len(SongData) == 0:
...
def get_comments(request):
    ...
    if Data["levelID"] == 0 or Data["levelID"] == "":
        ...
        DisplayID = True
        mycursor.execute(
            f"SELECT levelID, commentID, timestamp, comment, userID, likes, isSpam, percent FROM comments WHERE userID = %s ORDER BY {Column} DESC LIMIT %s OFFSET %s",
            (Data["userID"], Data["count"], Offset),
        )
        Comments = mycursor.fetchall()
        mycursor.execute(
            "SELECT count(*) FROM comments WHERE userID = %s", (Data["levelID"],)
        )
        CommentCount = mycursor.fetchall()[0][0]
    else:
        DisplayID = False
        mycursor.execute(
            f"SELECT levelID, commentID, timestamp, comment, userID, likes, isSpam, percent FROM comments WHERE levelID = %s ORDER BY {Column} DESC LIMIT %s OFFSET %s",
            (Data["levelID"], Data["count"], Offset),
        )
        Comments = mycursor.fetchall()
        mycursor.execute(
            "SELECT count(*) FROM comments WHERE levelID = %s", (Data["levelID"],)
        )
        CommentCount = mycursor.fetchall()[0][0]
    ...
    for Comment in Comments:
        ...
        mycursor.execute(
            "SELECT userID, userName, icon, color1, color2, iconType, special, extID FROM users WHERE userID = %s LIMIT 1",
            (Comment[4],),
        )
        UserData = mycursor.fetchone()
        if UserData == None:
            Fail(
                f"Failed to find data for user with the UserID of {Data['levelID']}! This will lead to a lot of bad things."
            )
        try:
...
def add_song_to_db(Response: str):
    ...
    mycursor.execute(
        "INSERT INTO songs (ID, name, authorID, authorName, size, download) VALUES (%s, %s, %s, %s, %s, %s)",
        (SongID, SongName, AuthorID, AuthorName, SongSize, SongURL),
    )
    mydb.commit()
...
def get_role_for_user(AccountID):
    ...
    Default = {"RoleID": 0, "RoleName": "User", "Badge": 0, "Colour": "256,256,256"}
    mycursor.execute(
        "SELECT privileges FROM accounts WHERE accountID = %s LIMIT 1", (AccountID,)
    )
    ...
    mycursor.execute(
        "SELECT id, name, colour FROM privilegegroups WHERE privileges = %s LIMIT 1",
        (UserPriv,),
    )
...
def delete_acc_comment(request):
    ...
    UserID = UserID[0]
    mycursor.execute(
        "DELETE FROM acccomments WHERE userID = %s AND commentID = %s LIMIT 1",
        (UserID, CommentID,),
    )
    mydb.commit()
    return "1"
...
def comment_command(Comment: str, Extra: dict) -> bool:
    ...
    if Command[0] == "setacc" and HasPrivilege(Extra["AccountID"], CommandSetAcc):
        mycursor.execute(
            "SELECT userID, extID FROM users WHERE isRegistered = 1 AND userName LIKE %s LIMIT 1",
            (Command[1],),
        )
        ...
        if User is None:
            return False
        mycursor.execute(
            "UPDATE levels SET userName = %s, userID = %s, extID = %s WHERE levelID = %s LIMIT 1",
            (Command[1], User[0], User[1], Extra["LevelID"]),
        )
        mydb.commit()
        return True
        ...
        mycursor.execute(
            "SELECT timestamp FROM dailyfeatures WHERE timestamp >= %s AND type = 0 ORDER BY timestamp DESC LIMIT 1",
            (Tmw,),
        )
        ...
        mycursor.execute(
            "INSERT INTO dailyfeatures (levelID, timestamp) VALUES (%s, %s)",
            (Extra["LevelID"], NewTimestamp),
        )
        mydb.commit()
        return True
    return False
...
def post_comment(request):
    ...
    if DeB64.startswith(UserConfig["CommandPrefix"]):
        return str(
            int(CommentCommand(DeB64, {"LevelID": LevelID, "AccountID": AccountID}))
        )  # cursed
    Timestamp = round(time.time())
    # we finally add the comment
    mycursor.execute(
        "INSERT INTO comments (levelID, userID, comment, timeStamp, percent, userName) VALUES (%s, %s, %s, %s, %s, %s)",
        (LevelID, UserID, Comment, Timestamp, Percent, Username),
    )
    mydb.commit()
...
class GDPySBot:
    ...
    def _set_user_id(self):
        ...
        mycursor.execute(
            "SELECT userID FROM users WHERE extID = %s LIMIT 1", (self.BotID,)
        )
    ...
    def _register_bot(self, BotName="GDPySBot"):
        ...
        mycursor.execute(
            "INSERT INTO accounts (userName, password, email, secret, saveData, registerDate, isBot) VALUES (%s, %s, 'rel@es.to', '', '', %s, 1)",
...
def CheatlessScoreCheck(Score: dict) -> bool:
    ...
    {"AccountID": 1, "LevelID": 222, "Percentage": 100, "Attempts": 10, "Coins": 0}
    if UserConfig["CheatlessAC"] and UserConfig["CheatlessScoreCheck"]:
        if Score["Percentage"] > 100:
            ...
            if (
                LevelData[1] == 10
                and LevelData[2] == 6
                and Score["Attempts"] < UserConfig["CheatlessExtremeDemonMinAttempts"]
            ):
...
def cheatless_ban(AccountID: int, Offence: str):
    ...
    Bot.SendMessage(
        ...
        Body=f"You have been by the Cheatless AntiCheat! The reason for your ban is {Offence}. Please contact the staff team for more info and stop cheating.",
    )
    mycursor.execute(
        "UPDATE users SET isBanned = 1 WHERE extID = %s LIMIT 1", (AccountID,)
    )
    mydb.commit()
...
def debug_manual_add_song(Source, SongID):
    ...
    SongInfo = requests.post(Source, data={"secret": "Wmfd2893gb7", "songID": SongID})
...
def message_post(request):
    ...
    mycursor.execute(
        "SELECT COUNT(*) FROM blocks WHERE person1 = %s AND person2 = %s LIMIT 1",
        (Target, AccountID),
    )
    ...
    mycursor.execute(
        "SELECT mS, userName FROM accounts WHERE accountID = %s", (Target,)
    )  # ill also squeeze the username in here
    ReturnThing = mycursor.fetchone()
    if ReturnThing[0] == 1:
    ...
    mycursor.execute(
        """INSERT INTO messages 
                            ...
                            (%s, %s, %s, %s, %s, %s, %s, %s, 0)
                            """,
        (AccountID, Target, Username, UserID, Secret, Subject, Body, Timestamp),
    )
    mydb.commit()
...
def api_get_level(LevelID):
    ...
    mycursor.execute(
        "SELECT userName, levelID, extID, userID, levelName, levelDesc, levelLength, audioTrack, songID, coins, starDifficulty, downloads, likes, starStars, uploadDate, awarded, magic, starDemon, starAuto, starFeatured, starEpic FROM levels WHERE isDeleted = 0 and levelID = %s LIMIT 1",
        (LevelID,),
    )
    LevelData = mycursor.fetchone()
    if LevelData == None:
    ...
    Comments = []
    mycursor.execute(
        "SELECT userID, userName, comment, timestamp, likes, percent FROM comments WHERE levelID = %s",
        (LevelID,),
    )
...
def get_messages(request):
    ...
    GetSent = int(
        request.form.get("getSent", 0)
    )  # 0 = msgs received, 1= msgs sent by the user
    ...
    mycursor.execute(
        f"SELECT * FROM messages WHERE {SQLCondition} = %s ORDER BY timestamp DESC LIMIT 10 OFFSET %s",
        (AccountID, Offset),
    )
    Messages = mycursor.fetchall()
    # count query
    mycursor.execute(
        f"SELECT COUNT(*) FROM messages WHERE {SQLCondition} = %s", (AccountID,)
    )
    ...
    for Message in Messages:
        ...
        mycursor.execute(
            "SELECT userName, userID FROM users WHERE extID = %s LIMIT 1",
            (Message[6 if GetSent else 4],),
        )
...
def get_message(request):
    ...
    mycursor.execute(
        "SELECT * FROM messages WHERE messageID = %s AND (toAccountID = %s OR accID = %s) LIMIT 1",
    ...
    if not Message[9]:
        mycursor.execute(
            "UPDATE messages SET isNew = 1 WHERE messageID = %s AND accID = %s LIMIT 1",
            (MessageID, AccountID),
        )
        mydb.commit()
    ...
    mycursor.execute(
        "SELECT userName, userID, extID FROM users WHERE extID = %s LIMIT 1",
        (Message[6 if GetSent else 4],),
    )
    ExtraUserData = mycursor.fetchone()
    UploadAgo = TimeAgoFromNow(Message[7])[:-4]
...
def delete_comment_handler(request):
    ...
    UserID = AIDToUID(AccountID)
    mycursor.execute(
        "DELETE FROM comments WHERE userID = %s AND commentID = %s LIMIT 1",
        (UserID, CommentID),
    )  # also squeezed a check that checks whether the person deleting the comment is that person into the query
    mydb.commit()
    return "1"
...
def map_pack_handler(request):
    ...
    Offset = 10 * int(
        request.form.get("page", 0)
    )  # turning page to an offset to be used in query below
    # mappack count
            ...
            JointStringBuilder(
                {
                    ...
                    "8": Mappack[
                        6
                    ],  # not gonna be using colors2. i dont even know what they are lmao
                }
            )
            + "|"
        )
...
def get_gauntlets_handler():
    ...
    for Gauntlet in Gauntlets:
        Levels = (
            f"{Gauntlet[1]},{Gauntlet[2]},{Gauntlet[3]},{Gauntlet[4]},{Gauntlet[5]}"
        )
        GauntletReturn += JointStringBuilder({"1": Gauntlet[0], "3": Levels}) + "|"
...
def user_calc_cp(UserID: int):
    ...
    # count rated levels
    mycursor.execute(
        "SELECT COUNT(*) FROM levels WHERE starStars > 0 AND userID = %s", (UserID,)
    )
    UserCP += mycursor.fetchone()[0]
    # count featured levels
    mycursor.execute(
        "SELECT COUNT(*) FROM levels WHERE starFeatured > 0 AND userID = %s", (UserID,)
    )
    UserCP += mycursor.fetchone()[0]
    # count epic levels
    mycursor.execute(
        "SELECT COUNT(*) FROM levels WHERE starEpic > 0 AND userID = %s", (UserID,)
    )
    UserCP += mycursor.fetchone()[0]
    # count magic levels
    if UserConfig["MagicGivesCP"]:
        mycursor.execute(
            "SELECT COUNT(*) FROM levels WHERE magic > 0 AND userID = %s", (UserID,)
        )
        UserCP += mycursor.fetchone()[0]
    # count awarded levels
    if UserConfig["AwardGivesCP"]:
        mycursor.execute(
            "SELECT COUNT(*) FROM levels WHERE awarded > 0 AND userID = %s", (UserID,)
        )
        UserCP += mycursor.fetchone()[0]

    # lastly we give the cp to them
    mycursor.execute(
        "UPDATE users SET creatorPoints = %s WHERE userID = %s LIMIT 1",
        (UserCP, UserID),
    )
    mydb.commit()
...
def score_submit_handler(request):
    ...
    # get prev scores
    mycursor.execute(
        "SELECT * FROM levelscores WHERE accountID = %s and levelID = %s LIMIT 1",
        (AccountID, LevelID),
    )
    ...
    if Score == None or Percent != 0:  # no prev scores
        ...
        if CheatlessScoreCheck(CheatlessStruct):
            mycursor.execute(
                "INSERT INTO levelscores (accountID, levelID, percent, uploadDate, coins, attempts) VALUES (%s, %s, %s, %s, %s, %s)",
    ...
    # dont do anything if the percentage is same or lower OR update it if percent is same and coin count is different
        ...
        if CheatlessScoreCheck(CheatlessStruct):
            mycursor.execute(
                "UPDATE levelscores SET percent = %s, uploadDate = %s, attempts = %s, coins = %s WHERE scoreID = %s LIMIT 1",
    ...
    if Type == 0:
        ...
        mycursor.execute(
            "SELECT * FROM levelscores WHERE levelID = %s ORDER BY percent DESC LIMIT 50",
            (LevelID,),
        )
        ...
        mycursor.execute(
            "SELECT * FROM levelscores WHERE levelID = %s AND uploadDate > %s ORDER BY percent DESC LIMIT 50",
            (LevelID, AfterTime),
        )
    ...
    mycursor.execute(
        "SELECT extID, userName, userID, icon, color1, color2, iconType, special FROM users WHERE extID in (%s) AND isBanned = 0",
        (SQLList,),
    )
...
def MaxStarCountBan() -> None:
    ...
    if UserConfig["CheatlessCronChecks"] and UserConfig["CheatlessAC"]:
        ...
        # ban em
        mycursor.execute(
            "UPDATE users SET isBanned = 1 WHERE stars > %s", (TotalStars,)
        )
        mydb.commit()

        logger.info(
            f"Done with {BannedCount} users banned! {round((time.time() - StartTime) * 1000, 2)}ms"
        )
...
def cache_level(LevelID: int) -> bool:
    ...
    if not os.path.exists(f"Data/Levels/{LevelID}"):
        ...
        mycursor.execute(
            "SELECT levelString FROM levels WHERE levelID = %s LIMIT 1", (LevelID,)
        )
...
def get_user_string(UID):
    """Returns user string."""
    mycursor.execute(
        "SELECT userName, extID, userID FROM users WHERE userID = %s LIMIT 1", (UID,)
    )
...
def dl_level(request):
    ...
    if (
        LevelID == -1
    ):  # would make into dict switch-like statement but it would be slower
        mycursor.execute(
            "SELECT levelID, feaID FROM dailyfeatures WHERE timestamp < %s AND type = 0 ORDER BY timestamp DESC LIMIT 1",
            (round(time.time()),),
        )
        a = mycursor.fetchone()
        LevelID = a[0]
        ...
        mycursor.execute(
            "SELECT levelID, feaID FROM dailyfeatures WHERE timestamp < %s AND type = 1 ORDER BY timestamp DESC LIMIT 1",
            (round(time.time()),),
        )
        a = mycursor.fetchone()
        LevelID = a[0]
    ...
    mycursor.execute(
        "UPDATE levels SET downloads = %s WHERE levelID = %s", (Downloads, LevelID)
    )
    mydb.commit()
    ...
    PasswordXor = 0
    if Level[10] != 0:
        PasswordXor = base64.b64encode(Xor(Level[10], 26364).encode("ascii")).decode(
            "ascii"
        )
...
def get_friends_list(AccountID: int):
    ...
    mycursor.execute(
        "SELECT accountID FROM friendreqs WHERE toAccountID = %s", (AccountID,)
    )
    Friends = mycursor.fetchall()
    mycursor.execute(
        "SELECT toAccountID FROM friendreqs WHERE accountID = %s", (AccountID,)
    )
...
def send_friend_req(request):
    ...
    # stupid checks
    mycursor.execute(
        "SELECT COUNT(*) FROM blocks WHERE (person1 = %s AND person2 = %s) OR (person1 = %s AND person2 = %s)",
        (AccountID, TargetAccountID, TargetAccountID, AccountID),
    )
    if mycursor.fetchone()[0]:
        return "-1"
    mycursor.execute(
        "SELECT COUNT(*) FROM friendships WHERE (person1=%s AND person2=%s) OR (person1=%s AND person2=%s)",
        (AccountID, TargetAccountID, TargetAccountID, AccountID),
    )
    if mycursor.fetchone()[0]:
        return "-1"
    mycursor.execute(
        "SELECT COUNT(*) FROM accounts WHERE accountID = %s AND frS = 1",
        (TargetAccountID,),
    )
    if mycursor.fetchone()[0]:
        return "-1"
    mycursor.execute(
        "SELECT COUNT(*) FROM friendreqs WHERE (accountID = %s AND toAccountID = %s) OR (accountID = %s AND toAccountID = %s)",
        (AccountID, TargetAccountID, TargetAccountID, AccountID),
    )
    if mycursor.fetchone()[0]:
        return "-1"
    mycursor.execute(
        "INSERT INTO friendreqs (accountID, toAccountID, comment, uploadDate) VALUES (%s, %s, %s, %s)",
        (AccountID, TargetAccountID, Message, round(time.time())),
    )
    mydb.commit()
    return "1"
...
def delete_friend_request(request):
    ...
    mycursor.execute(
        "DELETE FROM friendreqs WHERE accountID = %s AND toAccountID = %s",
        ((AccountID, Target), (Target, AccountID))[
            0 if int(request.form.get("isSender", 0)) else 1
        ],
    )
    mydb.commit()
    return "1"
...
def get_friend_req_list(request):
    ...
    mycursor.execute(
        "SELECT * FROM friendreqs WHERE {} = %s LIMIT 10 OFFSET %s".format(Col),
        (AccountID, Offset),
    )  # never format except when you trust the input 1000%
    FriendReqs = mycursor.fetchall()
    # check right here. dont bother wiht the count query if there
    if len(FriendReqs) == 0:
        return "-2"  # lonely
    mycursor.execute(
        "SELECT COUNT(*) FROM friendreqs WHERE {} = %s ".format(Col), (AccountID,)
    )
    Count = mycursor.fetchone()[0]
    if not GetSent:
        # mark em as read
        ...
        mycursor.execute(
            f"SELECT id FROM friendreqs WHERE {Col} = %s LIMIT 10 OFFSET %s",
            (AccountID, Offset),
        )
        AList = ""
        for x in mycursor.fetchall():
            AList += f"{x[0]},"
        mycursor.execute(
            "UPDATE friendreqs SET isNew = 0 WHERE id in (%s)", (AList[:-1],)
        )
        mydb.commit()
    ReturnStr = ""
    for Request in FriendReqs:
        mycursor.execute(
            "SELECT userName, userID, icon, color1, color2, iconType, special, extID FROM users WHERE extID = %s LIMIT 1",
            (Request[1] if GetSent else Request[0],),
        )  # TODO: turn these individual requests and turn them into
        UserData = mycursor.fetchone()
        ReturnStr += (
            JointStringBuilder(
                {
...
def tool_login_check(request) -> bool:
    ...
    mycursor.execute(
        "SELECT userName, password, accountID, privileges FROM accounts WHERE userName LIKE %s LIMIT 1",
        (Username,),
    )
    User = mycursor.fetchone()
    if User == None:
...
def get_daily(request):
    ...
    Timestamp = round(time.time())
    mycursor.execute(
        "SELECT levelID FROM dailyfeatures WHERE timestamp < %s AND type = %s ORDER BY timestamp DESC LIMIT 1",
        (Timestamp, Weekly),
    )
    ...
    if not Weekly:
        ...
        ).total_seconds()  # calculate time since midnight
        TimeToChange = (
            86400 - SecondsSinceMidnight
        )  # substract for seconds in day to get time til next midnight
    else:
        DayToday = datetime.today().weekday()
        EndTime = datetime.combine(datetime.today(), datetime.min.time()) + timedelta(
            days=7 - DayToday
        )  # new level every monday
