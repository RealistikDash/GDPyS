...
def welcome_sequence(no_ascii: bool = False):
    ...
    if not no_ascii:
        print(
            ASCII_ART.format(
                reset=Colours.reset, col1=random.choice(
                    Colours.all_col), col2=random.choice(
                    Colours.all_col), col3=random.choice(
                    Colours.all_col), col4=random.choice(
                        Colours.all_col), col5=random.choice(
                            Colours.all_col)))
    # No cache warning
...
def pre_run_checks():
    ...
    if not path.exists(
            user_config["level_path"]) or not path.exists(
            user_config["save_path"]):
        logging.error(lang.error("START_LVLPATH"))
        logging.info(
            lang.info(
                "SET_LVLPATH",
                user_config['level_path'],
                user_config['save_path']))
        raise SystemExit
...
def start_plugins():
    ...
    for plugin in os.listdir(homepath + "/plugins/"):
        if not path.isdir(
            homepath +
            "/plugins/" +
                plugin) and plugin.endswith(".py") and plugin != "__init__.py":
            plugin = plugin.strip(".py")
            print(f"Loading plugin \"{plugin}\".")
            plugins.append(plugin)
            Thread(target=lambda: importlib.import_module(
                "." + plugin, "plugins").setup()()).start()
...
async def init(loop):
    ...
    rate_limiter.add_to_struct("register", limit=2)
    # One IP may only try to login ten times a day.
    rate_limiter.add_to_struct("login", limit=10)
    return app
