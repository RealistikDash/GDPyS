...
class SongHelper:
    ...
    async def _top_artists(self) -> list:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "SELECT authorName AS frequency FROM songs GROUP BY authorName ORDER BY frequency DESC"
            )
    ...
    async def _song_from_db(self, song_id: int) -> Song:
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(
                "SELECT ID, name, authorID, authorName, size, download, isDisabled FROM songs WHERE ID = %s LIMIT 1",
                (song_id,),
            )
            song = await mycursor.fetchone()
        if song is None:
            return None
        return Song(song[0], song[1], song[2], song[3], song[4], song[5], bool(song[6]))
