...
async def quests_handler(request: aiohttp.web.Request):
    ...
    # We are assuming it is a cached object so we reduce DB calls.
    user = await user_helper.get_object(int(post_data["accountID"]))
    device_id = post_data["udid"]
    base_id = rewards_helper.gen_id()
    # This is so we can pop quests we already used without messing up the global quests.
    quests = await rewards_helper.get_quests()
    quest_str = []
    ...
    resp1 = encode_base64(cipher_xor(
        f"bruhh:{user.user_id}:{chk_decoded}:{device_id}:{user.account_id}:{tomorrow()-get_timestamp()}:{quest_str[0]}:{quest_str[1]}:{quest_str[2]}", XorKeys.quests))
