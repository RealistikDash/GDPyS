...
class ScoreHelper():
    ...
    async def get_from_db_filtered(self, level_id : int, filters : list):
        ...
        logging.debug(lang.debug("level_scores_gotten", level_id))
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute(f"SELECT scoreID, accountID, levelID, percent, uploadDate, attempts, coins FROM levelscores WHERE levelID = %s AND accountID IN ({safe_id_list(filters)})", (level_id,))
            return [self._score_obj_from_tuple(i) for i in await mycursor.fetchall()]
    
    async def save_score_to_db(self, score : Score) -> None:
        """Adds a score to db (NO CHECKS)."""
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute("INSERT INTO levelscores (accountID, levelID, percent, uploadDate, attempts, coins) VALUES (%s,%s,%s,%s,%s,%s)", (
                score.account_id, score.level_id, score.percentage, score.timestamp, score.attempts, score.coins
            ))
    ...
    async def delete_score(self, score_id : int):
        ...
        async with myconn.conn.cursor() as mycursor:
            await mycursor.execute("DELETE FROM levelscores WHERE scoreID = %s LIMIT 1", (score_id,))
            await myconn.conn.commit()
